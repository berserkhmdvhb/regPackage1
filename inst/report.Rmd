...
```{r}
#train[["outcome"]] <- lapply(train[["outcome"]], factor)
#train$outcome <- as.factor(train$outcome)
X_train <- train[, colnames(train)[colnames(train) != "outcome"]]
X_test <- test[, colnames(test)[colnames(test) != "outcome"]]
y_train <- as.factor(train[["outcome"]])
y_test <- as.factor(test[["outcome"]])
#
```
...
t <- tuneRF(X_train, 
            y_train, # esclude the response variable 
            stepFactor = 0.5, 
            plot = TRUE, 
            ntreeTry = 300, 
            trace = TRUE, 
...
eval <- eval_hmd(actual, predict_rf_num)
```
...
```{r}
pred_proba_rf <- predict(rf, X_test, type="prob")
pred_proba_rf_list <- pred_proba_rf |> as.list() |> tail(2000) |> unlist()
```
...
for (name in common_names){
  ...
  cols_revert <- cols_revert[lengths(cols_revert)!=0] |> unlist()
  
  w <- which(insurance_test[cols_revert]==1, arr.ind = T) 

  insurance_test_revert_name <- toupper(names(insurance_test[cols_revert])[w[order(w[,1]),2]])
  insurance_test$name <- insurance_test_revert_name
