...
df_racog <- racog(df_enc, numInstances = 3734, burnin = 100, lag = 20, classAttr = "outcome")
...
```{r}
table(df_racog$outcome)
```

```{r}
dplyr::glimpse(df_racog)
```
...
file_name = "car_insurance_data_processed.csv"
readr::write_csv(df_racog, 
...
```{r}
df_proc <- readr::read_csv(paste(path,file_name,sep=""))
```
